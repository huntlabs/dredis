...
private:
    ...
    import dredis.parser;
    import dredis.response;

debug(dredis) {
	import std.stdio : writeln;
	import dredis.encoder : escape;
}
...
public:
    ...
    {
        debug(dredis) { writeln("Request : '", escape(encoded_cmd) ~ "'"); }
    ...
    Response[] receiveResponses(TcpSocket conn, size_t minResponses = 0)
    {
        ...
        while(true)
        {
            ...
            debug(dredis) { writeln("BUFFER : ", escape(cast(string)buffer)); }
...
private :
    ...
    void receive(TcpSocket conn, ref byte[] buffer)
    {
        ...
        buffer ~= buff[0 .. len];
        debug(dredis) { writeln("Response : ", "'" ~ escape(cast(string)buffer) ~ "'", " Length : ", len); }
    }
